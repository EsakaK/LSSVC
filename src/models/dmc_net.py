...
class DMC(nn.Module):
    ...
    def forward_one_frame(self, x, ref_frame, ref_feature):
        ...
        mv_hat = self.mv_decoder(mv_y_hat)
        context1, context2, context3, warp_frame, ref_feature1, ref_feature2, ref_feature3 = self.motion_compensation(
            ref_frame, ref_feature, mv_hat)
        ...
        return {"bpp_mv_y": bpp_mv_y,
                ...
                "ref_feature3": ref_feature3,
                }

    def check(self, x, ref_frame):
        est_mv = self.optic_flow(x, ref_frame)
        mv_y = self.mv_encoder(est_mv)
        mv_z = self.mv_prior_encoder(mv_y)
        mv_z_hat = self.quant(mv_z)
    ...
    def get_rd_info(result):
        rd = {
            ...
            "me_mse": torch.mean(result['me_mse']),
            "mse_loss": torch.mean(result['mse_loss']),
            # "recon_ms_ssim": torch.mean(result['recon_ms_ssim']),
            # "me_ms_ssim": torch.mean(result['me_ms_ssim']),
            "recon_ms_ssim": 0,
            "me_ms_ssim": 0,
        }
        return rd
    ...
    def get_loss_info(rd):
        info = {
            "bpp_mv_y": rd['bpp_mv_y'].item(),
            "bpp_mv_z": rd['bpp_mv_z'].item(),
            "bpp_y": rd['bpp_y'].item(),
            "bpp_z": rd['bpp_z'].item(),
            "mse_loss": rd['mse_loss'].item(),
            "me_mse": rd['me_mse'].item(),
        }
        return info

    def forward(self, x, ref_frame, ref_feature, loss_func=None):
        if loss_func is None:
            return self.forward_one_frame(x, ref_frame, ref_feature)
        ...
        if frame_nums == 1:
            if ref_frame is None:
                ...
                ref_feature = None
            result = self.forward_one_frame(x, ref_frame, ref_feature)
        ...
        for frame_index in range(frame_nums):
            ...
            result = self.forward_one_frame(cur_frame, ref_frame, ref_feature)
            rd = self.get_rd_info(result)
            loss = loss_func(rd)
            losses.append(loss)
...
class VideoDataParallel(nn.parallel.DistributedDataParallel):
    ...
    def control_is_prediction_parameter(self, is_prediction=True, is_train=True):
        return self.module.control_is_prediction_parameter(is_prediction, is_train)

    def get_rec_only(self, x):
        return self.module.get_rec_only(x)
