...
from src.IntraModules.others import LowerBound
...
class EntropyBottleneck(EntropyModel):
    ...
    *tensorflow/compression*. See the original paper and the `tensorflow
    ...
    def forward(self, x):
        ...
        if self.is_RDO:
            means = self._medians().detach().view(1, -1, 1, 1)
            values = values - means
            noise = torch.round(values) - values
            noise = noise.clone().detach()
            outputs = values + noise
        ...
        if not self.is_RDO and self.training:
            outputs = self.d_quant(values, self._medians())
    ...
    *tensorflow/compression*. See the `tensorflow documentation
    ...
    def forward(self, inputs, scales, means=None):
        if self.is_RDO:
            ...
            noise = noise.clone().detach()
            outputs = values + noise
            likelihood = self._likelihood(outputs, scales, means=None)
        else:
            outputs = self._quantize(
            ...
            if self.train:
                outputs = self.d_quant(inputs, means)
        if self.use_likelihood_bound:
            likelihood = self.likelihood_lower_bound(likelihood)
        if self.is_RDO:
            outputs = outputs + means
